...
if args.remote:
    ...
    """
    cl_task = Task.init(
        project_name="PP_det2",
        task_name="pp_extract_upload",
        task_type="data_processing",
        output_uri="s3://ecs.dsta.ai:80/clearml-models/default/",
    )
    cl_task.set_base_docker(
        f"harbor.dsta.ai/public/detectron2:best-val-steps-nosavepred --env GIT_SSL_NO_VERIFY=true --env AWS_ENDPOINT_URL={AWS_ENDPOINT_URL} --env AWS_ACCESS_KEY={AWS_ACCESS_KEY} --env AWS_SECRET_ACCESS={AWS_SECRET_ACCESS} --env CERT_PATH={CERT_PATH} --env CERT_DL_URL={CERT_DL_URL}"
    )
    cl_task.execute_remotely(queue_name="pp-queue-1xV100-64ram", exit_process=True)
