...
def download_dir_from_s3(s3_resource, bucket_name, remote_dir_name, local_dir, unzip=True):
...
class S3_handler():
    def __init__(self, endpoint_url, aws_access_key_id, aws_secret_access_key, cert_path, local_weight_dir=None, local_data_dir=None):
    ...
    def dl_dirs(self, dirs, s3_bucket, s3_parent_path, local_parent, unzip=True):
        ...
        if dirs:
            ...
            for folder in dirs:
                ...
                download_dir_from_s3(self.s3_resource, s3_bucket, s3_dataset_path, local_folder_path, unzip=unzip)
